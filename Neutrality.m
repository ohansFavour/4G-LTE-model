...
% generation of users in base station
a=30;
b=240;
n=1000; 
...
for lamda = 0.5:1.5:12.5  % Adjust one on top too
 ...
 tNumCalls = 5000;  %Number of calls
 ...
 combineArrivalTime = sort(union(arrivalTimeNormal,arrivalTimePriority)); % all arrival times in Ascending form
 
 departureTime = inf*ones(1,tNumCalls);
 depTime = inf*ones(1,n); % for each user in the system
 
 tNum = 1;  % Current number of arrived calls
 channelStatus1 = zeros(1,numChannelsPerSector); % Available channels in cell1
 channelStatus2 = zeros(1,numChannelsPerSector); % Available channels in cell2
 channelStatus3 = zeros(1,numChannelsPerSector); % Available channels in cell3
 pp=0;
 ...
 minaggTime = 0; % initiating the aggregate time
  ...
  while(tNum<=tNumCalls-1)
    ...
    elseif priorityArrival == 1    % if user is from the Priority class
        allFree = find(uState == 0); % vector of indexes of all free users
        freeUsers = find(allFree > normal);
        uCall = allFree(freeUsers(randi(numel(freeUsers))));  % index of selected user to make call
    ...
    TF3 = isempty(id3); 
     ...
     elseif TF2==0  % if user to call is connected to cell2
        if (1<=uCall && uCall<= normal) % if user is a Normal user
     ...
     elseif TF3==0  % if user to call is connected to cell3
         if (1<=uCall && uCall<= normal) % if user is a Normal user
    ...
    if h==0 && m==0   % free channel and high enough snrDB
            ...
            if (cellSel(uCall) == 11 || cellSel(uCall) == 21)
              ...
              if(indexaggTime == 1) %if previous event was an arrival
                 Energy(l) = Energy(l) + power * (combineArrivalTime(tNum) - combineArrivalTime(tNum-1) );
              else  % if the previous event was a departure
            ...
            departureTime(tNum) = combineArrivalTime(tNum) + serviceTime(uCall); % Set new departure time
            
            depTime(uCall) = departureTime(tNum); % Set new departure time for user
     ...
     [minaggTime, indexaggTime] =  min(aggTime,[],1);
      ...
      if indexaggTime == 2 % Departure Time
         ...
         while i<=q
            ...
            if kk(i) ==11 || kk(i) == 21   % User from cell1
            ...
            elseif kk(i)==12 || kk(i) == 22  %User in cell2
            ...
            elseif kk(i)==13 || kk(i) == 23  %User in cell2
 ...
 blockingProb(l) = ((bNum(l))/(tNum))*100;
